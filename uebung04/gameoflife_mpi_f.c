...
int evolve(long t, double *currentfield, double *newfield, int w, int h) {
    int changing = 0;
    ...
    for (int row = 1; row < h - 1; ++row) {
        for (int col = 1; col < w - 1; ++col) {
            ...
            if (newfield[calcIndex(row, col, h)] != currentfield[calcIndex(row, col, h)]) {
                changing = 1;
            }
        }
    }
...
int main(int argc, char ** argv) {
    ...
    for (t = 0; t < TimeSteps; t++) {
        ...
        int globalChanges;
        int localChanges = evolve(t, currentfield, newfield, localWidth, localHeight);
        MPI_Allreduce(&localChanges, &globalChanges, 1, MPI_INT, MPI_SUM, comm);
